// Contributions of Kevin and Emile
//  Kevin: Implemented the hash function, Implemented the load function, Defined the node struct, Provided code documentation and
//  comments.
// Emile: Implemented the check function, Contributed to the unload function (memory deallocation), Implemented the size function,
// Assisted in testing and debugging, Collaborated on code review and refactoring
...
// TODO: Number of buckets in hash table
...
bool check(const char *word)
{
    ...
    int index = hash(word);
    // make a ptr node which points to the head of the hash table
...
bool unload(void)
{
    ...
    // make a ptr which points to the head node. this ptr will work through the linked lists making each = NULL, freeing the memory.
    node *tmp = NULL;
    node *ptr = NULL;
    for (int i = 0; i < TABLE_SIZE; i++)
    {
        // create a pointer that point to each index of the array
        ptr = table[i];
        while (ptr != NULL)
        {
            tmp = ptr;
